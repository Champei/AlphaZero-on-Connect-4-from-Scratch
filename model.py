...
def combined_loss(predicted_log_probs, predicted_values, target_policy, target_values,
                   net, policy_weight=1.0, value_weight=1.0, l2_weight=1e-4):
    ...
    total_loss = policy_weight * p_loss + value_weight * v_loss + l2_weight * l2_loss
    parts = {
        'policy': p_loss,
        'value': v_loss,
        'l2': l2_loss,
...
def self_play_iteration(net, optimizer, num_games, num_simulations, c_puct, batch_size, num_epochs=1, temperature=1.0):
    ...
    for _ in range(num_epochs):
        epoch_losses = training_epoch(net, optimizer, buffer, batch_size)
...
def match_win_rate(agent_one, agent_two, num_matches, alternate_starts=True):
    # Track wins, losses, and draws for agent_one
